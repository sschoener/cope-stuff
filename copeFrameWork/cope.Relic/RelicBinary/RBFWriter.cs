...
namespace cope.Relic.RelicBinary
{
    public class RBFWriter
    {
        ...
        /// <exception cref="RelicException">Trying to write unknown data-type!</exception>
        private static uint GetUInt32(AttributeValueType type)
        {
            switch (type)
            {
                ...
                case AttributeValueType.Table:
                    return 4;
                default:
                    throw new RelicException("Trying to write unknown data-type!");
            }
        }
        ...
        private uint WriteData(AttributeValue attribute)
        {
            if (m_bWriteRetributionFormat)
            {
                m_dataWriter.Write((ushort) GetUInt32(attribute.DataType));
        ...
        private uint GetIndexForKey(string key)
        {
            try
            {
                int idx = m_keyProvider.GetIndexForName(key);
                if (idx == -1)
