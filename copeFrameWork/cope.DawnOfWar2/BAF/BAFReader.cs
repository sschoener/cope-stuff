    ...
    public class BAFReader
    {
        ...
        private AttributeStructure ReadIntern(Stream str)
        {
            try
            {
                ...
                AttributeValue root = new AttributeValue(RelicAttribute.AttributeDataType.Table, m_sStrings[0], m_tables[0]);
                AssignDataToTables();
        ...
        private AttributeDataType GetType(uint type)
        {
            switch (type)
            {
                case 0:
        ...
        private AttributeValue[] ReadDataValues()
        {
            ...
            for (int i = 0; i < numDataEntries; i++)
            {
                ...
                RelicAttribute.AttributeDataType type = GetType(m_reader.ReadUInt32());
                ...
                switch (type)
                {
                    case RelicAttribute.AttributeDataType.Boolean:
                        value = (m_reader.ReadUInt32() == 1) ? true : false;
                        break;
                    case RelicAttribute.AttributeDataType.Float:
                        value = m_reader.ReadSingle();
                        break;
                    case RelicAttribute.AttributeDataType.Integer:
                        value = m_reader.ReadInt32();
                        break;
                    case RelicAttribute.AttributeDataType.String:
                        int stringIndex = (int) m_reader.ReadUInt32();
                        value = m_sStrings[stringIndex];
                        break;
                    case RelicAttribute.AttributeDataType.Table:
